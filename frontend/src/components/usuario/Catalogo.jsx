  ...
  useEffect(() => {
    const cargar = async () => {
      try {
        ...
        // Solo mostrar productos activos en catálogo
        // Activo_Catalogo es int(0/1) en MySQL - usar Number() para comparación segura
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content border-0 shadow">
              <div className="modal-header" style={{ backgroundColor: '#DB0000' }}>
                ...
                <button type="button" className="btn-close btn-close-white" onClick={() => setProductoSel(null)}></button>
              </div>
              <div className="modal-body">
                <div className="text-center mb-3" style={{ fontSize: '4rem' }}></div>
                <table className="table table-borderless">
                  <tbody>
                    ...
                    <tr><td className="fw-bold text-muted">Descripción</td><td>{productoSel.Descripcion}</td></tr>
                  </tbody>
                </table>
              </div>
              <div className="modal-footer border-0">
                <button className="btn text-white fw-bold" style={{ backgroundColor: '#DB0000' }}
                  onClick={() => { setProductoSel(null); setVista('preguntas'); }}>
                   Preguntar por este producto
                </button>
                <button className="btn btn-secondary" onClick={() => setProductoSel(null)}>Cerrar</button>
              </div>
            </div>
          </div>
        </div>
